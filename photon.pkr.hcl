...
//  sources = ["source.vmware-iso.vagrant-vmw", "source.virtualbox-iso.vagrant-vbx"]
  sources = ["source.vmware-iso.vagrant-vmw"]
  ...
  provisioner "shell" {
    scripts = [
      ...
      "${path.cwd}/scripts/system/move_files.sh",
    ]
  }
...
# substitute ovf file with the customized one
     post-processor "shell-local" {
        environment_vars  = ["BASEDIR=${path.cwd}"]
        execute_command   = ["bash", "-c", "{{.Vars}} {{.Script}}"]
        use_linux_pathing = false
        script            = "${path.cwd}/copy_ovf.sh"
